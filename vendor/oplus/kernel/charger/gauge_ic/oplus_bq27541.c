...
static bool zy0603_fg_check(void)
{
...
{
	if (!zy0603_fg_check()) {
		pr_info("%s zy0603_fg_check fail\n", __func__);
		return -1;
	}
...
static bool zy0603_check_fw_version(struct chip_bq27541 *chip, int *fwversion)
{
	u8 version_data[4] = {0};
...
static int zy0603_full_access(struct chip_bq27541 *chip)
{
...
int zy0603_reset_rc_sfr(void)
{
	...
	if (gauge_ic->fg_soft_version == -1) {
		mutex_lock(&bq28z610_alt_manufacturer_access);
		zy0603_check_fw_version(chip, &gauge_ic->fg_soft_version);
		mutex_unlock(&bq28z610_alt_manufacturer_access);
	}
