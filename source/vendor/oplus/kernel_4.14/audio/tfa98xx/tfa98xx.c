...
#ifdef OPLUS_ARCH_EXTENDS
// Kun.Zhao@MULTIMEDIA.AUDIODRIVER.PA, 2021/08/24, Add for spk reboot
...
#ifdef OPLUS_ARCH_EXTENDS
// Kun.Zhao@MULTIMUEDIA.AUDIODRIVER.PA, 2021/09/15, Add for dual tfa spk force mute
...
/*Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2021/06/03, Add for smartpa err feedback*/
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			tfa98xx->pstream = 1;
#ifdef OPLUS_ARCH_EXTENDS
/*Jianqing.Liao@PSW.MM.AudioDriver.SmartPA, 2021/11/01, Add for stereo spk*/
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	// Yongpei.Yao@MULTIMEDIA.AUDIODRIVER.PA, 2020/10/20, Add for spk force mute
	// Kun.Zhao@MULTIMUEDIA.AUDIODRIVER.PA, 2021/08/24, modification for dual tfa when adsp ssr
	if (init_times == 0) {
		snd_soc_add_codec_controls(tfa98xx->codec,
